...
class EncDecHybridRNNTCTCLIDBPEModel(EncDecHybridRNNTCTCModel, ASRBPEMixin):
    ...
    def __init__(self, cfg: DictConfig, trainer: Trainer = None):
        ...
        self.lid_loss_weight = self.cfg.aux_lid.get("aux_loss_weight", 0.3)

        self.lid_loss = CrossEntropyLoss(
            weight=self.cfg.aux_lid.get("loss_weights", [1.0] * self.cfg.aux_lid.num_classes))
        self.lid_acc = Accuracy(task="binary")

        lid_decoding_cfg = self.cfg.aux_lid.get('decoding', None)
        self.lid_decoding = EncDecClassificationModel.from_config_dict(lid_decoding_cfg)
